...
public class Consultation extends ActionSupport implements SessionAware {
	...
	private void afficherTournois() {
		tournois = daoFactory.getTournoiDAO().findByStatut(EStatutTournoi.TERMINE);
	}

	@SuppressWarnings("unchecked")
	private void afficherMatchsPoule() {
		tournoi = daoFactory.getTournoiDAO().findById((Integer) session.get("idTournoi"));
	...
	private void afficherMatchsArbreFinal() {
		...
		finale1 = daoFactory.getMatchDAO().find(tournoi, ENiveauMatch.FINALE1).get(0);
		finale2 = daoFactory.getMatchDAO().find(tournoi, ENiveauMatch.FINALE3).get(0);
		finale3 = daoFactory.getMatchDAO().find(tournoi, ENiveauMatch.FINALE5).get(0);
		finale4 = daoFactory.getMatchDAO().find(tournoi, ENiveauMatch.FINALE7).get(0);
	}

	private void afficherButeurs() {
		tournoi = daoFactory.getTournoiDAO().findById((Integer) session.get("idTournoi"));

		List<Evenement> buts = daoFactory.getEvenementDAO().find(tournoi, ETypeEvenement.BUT);
	...
	private void afficherBlesses() {
		...
		List<Evenement> blessures = daoFactory.getEvenementDAO().find(tournoi,
	...
	private void afficherBouchers() {
		...
		Set<Joueur> temp = new HashSet<Joueur>();
		List<Evenement> jaunes = daoFactory.getEvenementDAO().find(tournoi,
				ETypeEvenement.CARTON_JAUNE);
		for (Evenement jaune : jaunes) {
			temp.add(jaune.getJoueur());
		}

		List<Evenement> rouges = daoFactory.getEvenementDAO().find(tournoi,
