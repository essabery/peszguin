...
public class Evolution extends ActionSupport implements SessionAware {
	...
	public String initialiser() {
		...
		List<Tournoi> all = daoFactory.getTournoiDAO().findAll();
		for (Tournoi t : all) {
	...
	public String choixTournoi() {
		if (idTournoi != -1) {
			Tournoi tournoi = daoFactory.getTournoiDAO().findById(idTournoi);
	...
	public String phase1Init() {
		Tournoi tournoi = daoFactory.getTournoiDAO().findById(
				(Integer) session.get(cleTournoi));
		...
		e1 = daoFactory.getEquipeDAO().findById(equipes.get(0).getId());
		e2 = daoFactory.getEquipeDAO().findById(equipes.get(1).getId());
		e3 = daoFactory.getEquipeDAO().findById(equipes.get(2).getId());
		e4 = daoFactory.getEquipeDAO().findById(equipes.get(3).getId());
		e5 = daoFactory.getEquipeDAO().findById(equipes.get(4).getId());
		e6 = daoFactory.getEquipeDAO().findById(equipes.get(5).getId());
		e7 = daoFactory.getEquipeDAO().findById(equipes.get(6).getId());
		e8 = daoFactory.getEquipeDAO().findById(equipes.get(7).getId());
	...
	public String phase1Valider() {
		...
		e1 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe1"));
		e2 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe2"));
		e3 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe3"));
		e4 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe4"));
		e5 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe5"));
		e6 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe6"));
		e7 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe7"));
		e8 = daoFactory.getEquipeDAO().findById(
				(Integer) session.get("idEquipe8"));
		...
		Participant participant1 = daoFactory.getParticipantDAO().findById(p1);
		Participant participant2 = daoFactory.getParticipantDAO().findById(p2);
		Participant participant3 = daoFactory.getParticipantDAO().findById(p3);
		Participant participant4 = daoFactory.getParticipantDAO().findById(p4);
		Participant participant5 = daoFactory.getParticipantDAO().findById(p5);
		Participant participant6 = daoFactory.getParticipantDAO().findById(p6);
		Participant participant7 = daoFactory.getParticipantDAO().findById(p7);
		Participant participant8 = daoFactory.getParticipantDAO().findById(p8);
		...
		daoFactory.getMatchDAO().insert(m11);
		daoFactory.getMatchDAO().insert(m12);
		daoFactory.getMatchDAO().insert(m13);
		daoFactory.getMatchDAO().insert(m14);
		daoFactory.getMatchDAO().insert(m21);
		daoFactory.getMatchDAO().insert(m22);
		daoFactory.getMatchDAO().insert(m23);
		daoFactory.getMatchDAO().insert(m24);
		daoFactory.getMatchDAO().insert(m31);
		daoFactory.getMatchDAO().insert(m32);
		daoFactory.getMatchDAO().insert(m33);
		daoFactory.getMatchDAO().insert(m34);
		daoFactory.getMatchDAO().insert(m41);
		daoFactory.getMatchDAO().insert(m42);
		daoFactory.getMatchDAO().insert(m43);
		daoFactory.getMatchDAO().insert(m44);
		daoFactory.getMatchDAO().insert(m51);
		daoFactory.getMatchDAO().insert(m52);
		daoFactory.getMatchDAO().insert(m53);
		daoFactory.getMatchDAO().insert(m54);
		daoFactory.getMatchDAO().insert(m61);
		daoFactory.getMatchDAO().insert(m62);
		daoFactory.getMatchDAO().insert(m63);
		daoFactory.getMatchDAO().insert(m64);

		daoFactory.getEquipeDAO().updateTirage(e1.getId(), p1, poule1, pos1);
		daoFactory.getEquipeDAO().updateTirage(e2.getId(), p2, poule2, pos2);
		daoFactory.getEquipeDAO().updateTirage(e3.getId(), p3, poule3, pos3);
		daoFactory.getEquipeDAO().updateTirage(e4.getId(), p4, poule4, pos4);
		daoFactory.getEquipeDAO().updateTirage(e5.getId(), p5, poule5, pos5);
		daoFactory.getEquipeDAO().updateTirage(e6.getId(), p6, poule6, pos6);
		daoFactory.getEquipeDAO().updateTirage(e7.getId(), p7, poule7, pos7);
		daoFactory.getEquipeDAO().updateTirage(e8.getId(), p8, poule8, pos8);

		daoFactory.getTournoiDAO().updateStatut(tournoi.getId(),
				EStatutTournoi.POULES);
	...
	public String modifMatch() {
		phase2Affichage();
		modif = true;
		Match matchEnCours = daoFactory.getMatchDAO().findById(idMatchEnCours);
	...
	public String addEvenement() {
		...
		if (idJoueur == -1) {
			evenement.setJoueur(daoFactory.getJoueurDAO().findById(-1));
			evenement.setEquipe(matchEnCours.getEquipeDomicile());
		} else if (idJoueur == -2) {
			evenement.setJoueur(daoFactory.getJoueurDAO().findById(-1));
			evenement.setEquipe(matchEnCours.getEquipeExterieur());
		} else {
			Joueur joueur = daoFactory.getJoueurDAO().findById(idJoueur);
		...
		daoFactory.getEvenementDAO().insert(evenement);
		matchEnCours = daoFactory.getMatchDAO().findById(matchEnCours.getId());
		session.put("matchEnCours", matchEnCours);
	...
	public String resetMatch() {
		...
		Match matchEnCours = (Match) session.get("matchEnCours");
		matchEnCours = daoFactory.getMatchDAO().findById(matchEnCours.getId());
		daoFactory.getEvenementDAO().deleteByMatch(matchEnCours);
		matchEnCours.setTermine(false);
		daoFactory.getMatchDAO().updateMatchReset(matchEnCours);
		matchEnCours = daoFactory.getMatchDAO().findById(matchEnCours.getId());
		initListesEvenement(matchEnCours);
	...
	public String validerMatch() {
		...
		daoFactory.getMatchDAO().updateMatchTermine(matchEnCours);
		Collections.sort(equipesPouleA, new EquipeComparatorPoule());
		Collections.sort(equipesPouleB, new EquipeComparatorPoule());
		return SUCCESS;
	}
	...
	private void phase2Affichage() {
		...
		List<Match> matchs = daoFactory.getMatchDAO().find(tournoi);
		...
		for (Match match : matchs) {
			if (match.isPoule()) {
				...
				matchsParNiveauEtPoule.get(match.getNiveau()).get(
						match.getEquipeDomicile().getPoule()).add(match);
			}
		}
	...
	private void initListesEvenement(Match matchEnCours) {
		typesEvenements = new ArrayList<ETypeEvenement>(Arrays
				.asList(ETypeEvenement.values()));
	...
	public String phase2Valider() {
		...
		daoFactory.getMatchDAO().insert(m1);
		daoFactory.getMatchDAO().insert(m2);
		daoFactory.getMatchDAO().insert(m3);
		daoFactory.getMatchDAO().insert(m4);
		daoFactory.getMatchDAO().insert(m5);
		daoFactory.getMatchDAO().insert(m6);
		daoFactory.getMatchDAO().insert(m7);
		daoFactory.getMatchDAO().insert(m8);

		daoFactory.getTournoiDAO().updateStatut(tournoi.getId(),
				EStatutTournoi.ARBRE_FINAL);
		return SUCCESS;
	}
	...
	private void phase3Affichage() {
		...
		List<Match> matchsDemiAller = daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.DEMI_ALLER);
		List<Match> matchsDemiRetour = daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.DEMI_RETOUR);
		List<Match> matchsFinale = new ArrayList<Match>();

		matchsFinale.addAll(daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.FINALE1));
		matchsFinale.addAll(daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.FINALE3));
		matchsFinale.addAll(daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.FINALE5));
		matchsFinale.addAll(daoFactory.getMatchDAO().find(tournoi,
				ENiveauMatch.FINALE7));
		...
		if (demi1Aller.isTermine() && demi2Aller.isTermine()
			...
			if (matchsFinale.isEmpty()) {
				...
				daoFactory.getMatchDAO().insert(finale1);
				daoFactory.getMatchDAO().insert(finale2);
				daoFactory.getMatchDAO().insert(finale3);
				daoFactory.getMatchDAO().insert(finale4);
			} else {
				finale1 = findMatchByEquipe(matchsFinale, getVainqueur(
						demi1Aller, demi1Retour));
				finale2 = findMatchByEquipe(matchsFinale, getPerdant(
						demi1Aller, demi1Retour));
				finale3 = findMatchByEquipe(matchsFinale, getVainqueur(
						demi3Aller, demi3Retour));
				finale4 = findMatchByEquipe(matchsFinale, getPerdant(
						demi3Aller, demi3Retour));
			}
